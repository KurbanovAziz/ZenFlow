...
class OnGoingWorkOut extends StatefulWidget {
  OnGoingWorkOut({super.key});

  @override
  State<OnGoingWorkOut> createState() => _OnGoingWorkOutState();
}

class _OnGoingWorkOutState extends State<OnGoingWorkOut> {
  ...
  void initState() {
    ...
    appBartext = Get.arguments['videotype'];
    isSchedule = Get.arguments['isSchedules'];
    videoID = Get.arguments['video_id'];
    playMusic();
    totalSavasanaMinutes =
        int.parse(Get.arguments['savasanaTime'].toString().split(' ').first);
    urlToVideoId = YoutubePlayer.convertUrlToId(Get.arguments['videoLink'])!;
    print("video $urlToVideoId");
    print("isSchedules ${Get.arguments["isSchedules"]}");
    print("video_id ${Get.arguments["video_id"]}");

    _controller = YoutubePlayerController(
      initialVideoId: urlToVideoId,
          ...
          if (savasanaSeconds > 0) {
            ...
            if (savasanaMinutes > 0) {
              ...
              if (savasanaHours > 0) {
                ...
                isSchedule == true
                    ? calendercontroller.updateScheduleVideoStatus(
                        id: videoID,
                        workoutStatus: ScheduleStatus.success.name.toString(),
                      )
                    : customSnackBar(title: '${Get.arguments["isSchedules"]}');
              }
            }
          }
        } else {
